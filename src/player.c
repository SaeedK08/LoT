...
void update_player() {
    ...
    if (keyboard_state[SDL_SCANCODE_A] && dest_position.x>0) {
        ...
        src_position.x += 128;  
        if (src_position.x > 700) src_position.x = 56.0f; 
        //SDL_Delay(1000/60); 
        // position.x --;
    }
...
bool init_player(SDL_Renderer* renderer) {
    char anim_path[] = "Idle_Animation.gif";
    char walk_anim_path[] = "Walk_animation.gif";
    char walk_texture[] = "modified_walk.png";
    ...
    for (int i = 0; i < anim->count; i++) player_animTextures[i] = SDL_CreateTextureFromSurface(renderer,anim->frames[i]);
    for (int i = 0; i < walk_anim->count; i++) player_walkAnimTextures[i] = SDL_CreateTextureFromSurface(renderer,walk_anim->frames[i]);
    return true;
}
